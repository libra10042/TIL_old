// 최장 경로 찾기
/*
첫 줄에는 다음과 같은 집합 자료형 형태로 노드의 연결 관게가 주어집니다.
두번째 줄에는 경로를 구할 두 정점의 번호가 공백으로 구분되어 주어집니다. 이 두 정점으로 가기 위한 최대 거리를 구하고자 합니다.

최대 거리란, 정점의 중복 없이 한 정점에서 다른 정점까지 경유할 수 있는 가장 많은 간선의 수를 의미합니다.
...
function solution(n, visited){
    ...
    for(let next_node in graph[node]){
        n.push(graph[node][nex_node]);
        max.push(length, sol(n, cisited));
