...
--(DBA계정-SYSTEM) CONNECT, RESOURCE 롤에 대한 권한 확인
SELECT * FROM dba_sys_privs WHERE GRANTEE IN ('CONNECT', 'RESOURCE');
--------------------
--(롤 생성 SYSTEM계정) PROGRAMMER(개발자)라는 역할(롤, ROLE) 생성
CREATE ROLE PROGRAMMER;

--(SYSTEM) PROGRAMMER 롤에 테이블, 뷰(VIEW) 생성 권한 담기
GRANT CREATE ANY TABLE, CREATE ANY VIEW TO PROGRAMMER;

--(SYSTEM) MDGUEST 유저에게 PROGRAMMER 롤 부여
